...
function gateKeeper(req, res, next) {
  // Object.assign turns the object into a clean, readable format. Sets default to user = null and pass = null, then parses the string and if there are values replaces null with those values
  const {user, pass} = Object.assign(
    {user: null, pass: null}, queryString.parse(req.get('x-username-and-password')));
 
  req.user = USERS.find(
    (usr, index) => usr.userName === usr && usr.password === pass);
